...
2. **System files, project-wide.** `.interface-design/system.md`, `system.md`, `design-system.md`, and any `CLAUDE.md` section whose heading matches `-i '(token|colou?r|spacing|typograph|pattern|component|copy|voice|writing|tone)'` → those sections only, at most eighty lines each. Any sentence in them containing "always", "all", "must", or "never" that describes a visual pattern or a copy rule is recorded into `mandated_patterns` with its section cite; a copy rule binds write mode the way a mandated stroke binds review.
