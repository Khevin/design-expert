...
Discovery is bounded. Start with roughly fifteen hundred words of relevant token and pattern sections and stop when the task's constraints are clear. Expand when a referenced dependency, exception, or unresolved requirement changes the decision; a reading budget must not hide an applicable constraint. A discovery pass that reads the whole repository has replaced the design work with archaeology.

---

## The probes

Two kinds of source answer two different questions. System files answer "what has this project decided for every surface"; a brief answers "what has this surface decided for itself". Both are collected when present. Everything after them is a fallback for projects that have neither, and the fallbacks stop at the first tier that answers. Prefer relevant token and pattern sections, expanding as required to understand their dependencies. Read applicable instruction files in full when the active environment requires it; these heuristics never override that requirement.
...
2. **System files, project-wide.** `.interface-design/system.md`, `system.md`, `design-system.md`, and any `AGENTS.md` or `CLAUDE.md` section whose heading matches `-i '(token|colou?r|spacing|typograph|pattern|component|copy|voice|writing|tone)'` → the relevant sections, following continuations rather than truncating an applicable rule; plus any file those instruction files name as the design system, token spec, or canonical reference. Search them for `system.md`, `tokens`, and `design system`, because the file they call canonical outranks the one this list happens to name. Any rule stated as an absolute, in whatever language the file is written (always, never, must, all; sempre, nunca, deve, todos; a Do or a Don't bullet; a rule marked as voted or rejected), that describes a visual pattern or a copy rule is recorded into `mandated_patterns` with its section cite; a copy rule binds write mode the way a mandated stroke binds review.
...
7. **Connected design workspace.** When the active harness exposes a project-based design canvas or design-system connector, list or inspect its projects once, match by project name, or ask which. Use only capabilities actually present in the session; `harnesses.md` names the harness-specific options.
8. **Nothing found.** Ask, once, with the orange marker:

> 🟠 **Question**: How is the design system implemented here: tokens in code (which file?), a Figma library (link?), another connected design workspace, or nothing yet? If nothing, we design from the brief and I propose the first tokens.
