...
| Seat model | The model option of the spawn tool when it offers one, otherwise the active model | `model` on the `Agent` call; the agent file says `inherit` and `effort: high`, and only supporting seats in a large room pass `opus` | Whatever runs the seat; never more than one model below the session |
