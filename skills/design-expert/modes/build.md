...
Where the alternatives are shown follows `targets.md`. At redesign size, or when an iteration shifts the layout or the user asks to see alternatives, render them on the resolved concept surface: Figma frames through `use_figma` for a designer with the MCP, otherwise one labeled artboard or preview per alternative in the available visual workspace. At polish and at iterations that keep the layout, the alternatives are inline prose, then the pick. At capsule tier the consultation happens here: seat by tag from `design-gods.md` and write one sentence per seat naming the principle and the decision it shapes.

### Gate 8½: Council

Surface redesign and system redesign only, per `council.md`. Assemble the brief: mode build, size, register, the surface, two to four surface tags, the scene sentence, the artifact (the Gate 8 alternatives as a screenshot or summary, never a directory), and the decisions: the Gate 8 directions, the layout pick, the signature element, the defaults to reject. When parallel seats are available and allowed, seat the three permanent seats plus every designer whose tags intersect the surface tags, seven at minimum, in one wave. Run the chair, print the table, ask about Contested decisions in one structured-choice interaction, and write the log when permitted. Otherwise use the capsule fallback. Rulings are adopted before Gate 9 begins.
