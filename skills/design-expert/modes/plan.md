...
Render the probes on the concept surface triage chose in `targets.md`. For a designer with the Figma MCP, that is two to four frames on one page of the project's Figma file, made through `use_figma` after `figma:figma-use`, bound to the library's variables where a library exists. For everyone else, use the resolved visual workspace with the brief, the register, and the named directions, one labeled artboard or preview per direction. Only when no visual surface is available describe the directions in prose with named real-world references, actual products and printed objects, never adjectives like "modern" or "clean". If two probes feel close, kill one and replace it with something further away; the value is in distance.

### Gate 3½: Council

Every plan receives council consultation, sized to its decisions rather than a fixed headcount. Planning is high-leverage: cover the material constraints and meaningful counterpoints, and honor an explicit `council: full` setting. The procedure is in `council.md`; this gate applies it.

First draft the layout candidates Gate 4 will present (walk `layouts.md` for the register and pick three to five), so the room can rule on them. Then assemble the brief: mode plan, register, the surface, the relevant surface tags and aliases, the scene sentence, the artifact (the picked probe as a screenshot or a summary), and an initial decision set, extended or narrowed when the brief warrants it: D1 the visual direction picked at Gate 3; D2 the drafted layout candidates; D3 the type posture; D4 the density and elevation posture; D5 the proposed restrictions. When parallel seats are available and allowed, select seats and depth using `council.md`, dispatch within the concurrency limit, run the chair, and print the decision summary. Ask about Contested decisions in manageable batches; keep unanswered choices pending. Otherwise run the capsule fallback from `council.md` and continue. Write the log only when the resolved state location is writable.
