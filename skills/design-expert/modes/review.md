...
Depth follows the scope of what is reviewed, which triage sets in place of a change size, never time pressure. **Light** (one component, a microcopy edit, a small diff) is three sections, Context, Issues with severity tiers, References, at two to five hundred words, usually with zero or one capsule seat, expanded only for distinct material concerns. **Sub-surface** (one screen inside a larger flow) is four sections, Context, Issues, anti-defaultism scan, Next Steps, at five hundred to twelve hundred words, citing the parent audit, with a capsule consultation sized to the decisions, often around three seats. When no parent audit exists, walk Universal Design 7 in one line per principle rather than skipping it, and say in Context that no parent audit exists. **Full** (a primary flow, a new feature, a brand launch, a system-level change) is all eight sections from `output-format.md` with full citation discipline, the numeric score, the confidence framework, and the council at Step 5½. A bad full review on a microcopy edit teaches the team that reviews are bureaucratic; a light review on a primary flow ships broken work.
...
Full depth only, per `council.md`. Assemble the brief: mode review, size, register, the surface, the relevant surface tags and aliases, the scene sentence if a brief exists, the artifact (the file, the screenshot, or a summary, never a directory), and the decisions: the HOW of each Blocker and Major found so far, highest severity first, grouped into manageable rounds without dropping remaining issues. Select seats for coverage and relevant counterpoints using `council.md`; dispatch within the active concurrency limit or disclose the capsule fallback. Run the chair. Rulings become the HOW of the corresponding Issues, cited with the seat ("HOW, per council: ranked bars; veto adopted, edward-tufte"). Contested decisions become Open Questions carrying the substantive tradeoffs and unresolved evidence; any council-backing percentages are not probabilities of correctness. Dissents are recorded in the References section. Write the log when permitted. At light and sub-surface depth, a capsule consultation at Step 1 replaces the room: relevant seats, with enough explanation to connect each principle to a concrete decision.
