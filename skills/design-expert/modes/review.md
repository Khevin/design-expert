...
Depth follows the size triage assigned, not time pressure. **Light** (touch-up or polish scope: a single component, a microcopy edit, a small diff) is three sections, Context, Issues with severity tiers, References, at two to five hundred words, with a capsule consultation of one seat at most. **Sub-surface** (iteration scope: one screen inside a larger flow) is four sections, Context, Issues, anti-defaultism scan, Next Steps, at five hundred to twelve hundred words, citing the parent audit, with a capsule consultation of three seats. **Full** (redesign scope: a primary flow, a new feature, a brand launch, a system-level change) is all eight sections from `output-format.md` with full citation discipline, the numeric score, the confidence framework, and the council at Step 5½. A bad full review on a microcopy edit teaches the team that reviews are bureaucratic; a light review on a primary flow ships broken work.
