...
**The plan step consults the full pantheon — always.** Whenever `/design-expert:plan` runs — and whenever planning-shaped work happens without the command (a brief, a direction doc, a spec that will govern later builds) — load every designer file in `design-gods/`, the complete roster, extract at least one principle from each that bears on the surface being planned, and log every consultation. Planning is the highest-leverage moment in the workflow: a voice skipped at plan time is a constraint the brief never encodes and the build never inherits. There is no sizing exception at plan time. Trivial plans do not exist; if the work is trivial, it should not be a plan.

For build, review, and write, the floor scales with the work — and it is a floor, not a target:
...
- **Small — one god minimum.** A single contained component: a modal, a card, a form field, a status pill, an isolated hover treatment. Consult the designer whose principle is load-bearing; add a second when in doubt.
- **Medium — three gods minimum, five preferred.** A feature surface that holds multiple components or that touches more than one screen — a settings panel, a dashboard section, a marketing hero, a navigation system, a populated empty state, a content-rich card grid. Three consultations are the floor; five are the bar.
- **Large — five gods minimum; prefer the full pantheon.** A full page, a full flow, a system-level decision (token system, type scale, component library). Work at this size feeds the same decisions a plan would; treat it with plan-grade consultation and reach for the complete roster when the surface will govern later work.

If you cannot pick the relevant gods, the file index in `design-gods.md` exists for exactly that — scan the capsules, identify which voices speak to the surface, and load those depth files. "I couldn't think of one" is not a defensible answer when the full pantheon index sits one file away.
...
| `/design-expert:plan` | Generate `PRODUCT.md` and `DESIGN.md` for a project. Consults the full design-gods pantheon, always. Loader check is mandatory — never silent overwrite. | Starting a new project; refreshing a stale brief. |
...
- **The full pantheon on every plan; scaled floors elsewhere.** `/design-expert:plan` — and any planning-shaped work — consults every designer in `design-gods/`, no sizing exception. Build/review/write floors: one god on small work, three on medium (five preferred), five on large (full pantheon preferred). Every consultation logs to `~/.claude/design-expert/usage-log.jsonl`. See the "Consulting the design-gods" section above for the sizing definitions, the four-step consultation procedure, and the log format.
