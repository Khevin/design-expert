...
This is why components are not "reusable code." They are agreed-upon contracts about meaning. The contract for a status pill is: this color, paired with this icon and this label, means exactly this thing. The contract for an icon is: this glyph, at this size and stroke weight, means exactly this concept across the product. The contract for a chart is: this shape carries this question; if your data is a different shape, this is the wrong chart. The smaller the contract's surface area, the more important the precision — because the smaller it is, the more often it repeats, and repetition is where drift compounds. The trap is treating these atoms as cosmetic. The user spends seconds on your page layout and minutes on your status pills, your icons, your numbers. Get the layout slightly wrong and the user adjusts. Get the atoms wrong, repeatedly, and the user concludes your product is sloppy without ever knowing why.
...
## Icons — and the no-emoji mandate
...
Pick a single icon library — Lucide, Heroicons, Phosphor, Tabler, or Material Symbols are all defensible choices — and use it everywhere. The visual rhythm of the system depends on every icon coming from the same hand. Open-source, comprehensive, well-named libraries with good framework coverage (React, Vue, Svelte, vanilla web components) are the right starting points. Do not mix Lucide with Phosphor or Heroicons or Material Symbols on the same product — pick one, commit, and stick with it.
...
Stroke weight is the parameter most teams ignore and the one that most defines the product's feeling. Most icon libraries ship with a default stroke of two pixels. The common system runs from 1.5 to 2.5 in quarter-pixel increments, and each weight carries a register.

A 1.5 stroke is refined and editorial — light, considered, magazine-like. Right for content-driven products, brand surfaces, reading apps, and editorial surfaces (case studies, long-form essays, museum sites). A 2.0 stroke is the default for general SaaS — clear, neutral, hard-working. Most product UI lives here. A 2.5 stroke is technical and assertive — right for developer tools, terminals, dense engineering UIs where icons must read against high information density. The icons must visually rhyme with the type — a heavy stroke against a light typeface reads as mismatched, and vice versa. Within a single product, pick one weight and use it everywhere. Flipping between weights mid-product is the icon equivalent of flipping fonts mid-paragraph.

The register-conditional rule is explicit: **brand 1.5–2.0**, **product 1.75–2.0** (use 2.0 in dense data, 1.75 where the surface earns lighter chrome), **editorial 1.5** for refinement. When the active register is editorial, override the project's product default and ship at 1.5 — heavier strokes feel out of register against editorial type and grid. See `styles/editorial.md` for the canonical editorial discipline.
...
The rule: pick from this canonical mapping; if you need a concept not on the list, add it to the project's mapping file before using it. Never `import { Trash } from 'your-icon-library'` directly in a feature — import from a project-level icon file that re-exports the canonical choices. This makes the contract enforceable in code review and renders future migration trivial.
