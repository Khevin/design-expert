...
Use `block` only when shipping the decision as stated would violate your designer's principle in a way a user would feel. If `god_file` cannot be read, return `verdict: approve`, `confidence: 0`, `evidence: file-missing`. Keep the block under one hundred and twenty words. Write `change` and `evidence` as plain sentences without dashes.
